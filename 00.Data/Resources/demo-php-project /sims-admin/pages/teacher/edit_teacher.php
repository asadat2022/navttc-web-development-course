<?php
  ...
  $editSQL="SELECT * FROM `std_manage_system`.`Teacher` WHERE teacher_ID='".$editid."'";
  $editrecord=mysqli_query($dbconnect,$editSQL);
  $editrow=mysqli_fetch_array($editrecord);

  $edituser=$editrow["teacher_ID"];
  $editfname=$editrow["teacherFirstName"];
  $editlname=$editrow["teacherLastName"];
    ...
    if (isset($_POST["btn-submit"])) {
      if ($_SESSION["pass"]===$_POST["cpass"]) {
        ...
        if ($editdep==='Choose Department') {
          ...
          window.location.href="admin_panal.php?page=9&editstd='.$editid.'";
          </script>';
        }

        $SQL="UPDATE `std_manage_system`.`Teacher` SET teacher_ID ='$edituser', teacherFirstName ='$editfname', teacherLastName='$editlname', DepartmentName='$editdep' WHERE teacher_ID='$editid'";
        $usertest = mysqli_query($dbconnect,$SQL);
        if ($usertest>0) {
          if (!empty($_POST["npass"])&&!empty($_POST["rpass"])) {
            if ($_POST["npass"]===$_POST["rpass"]) {
              $editpass=$_POST["npass"];
              $SQL="UPDATE `std_manage_system`.`Teacher` SET teacherPassword ='$editpass' WHERE teacher_ID='$editid'";
              mysqli_query($dbconnect,$SQL);
              echo '<script>
              alert("Teacher Updated");
              window.location.href="admin_panal.php?page=6";
              </script>';
            }else {
              echo '<script>
 ...
 <div class="content">
     ...
     <form method="post" name="editadmin">
       <table class="addtable">
         <tr>
           <td>Teacher ID</td>
           <td><input type="text" class="addinput" name="stdid" required="required" pattern="([A-Z]{2,6})-([0-9]{3})([A-Z]{1})([0-9]{2})-([0-9]{1,})" value="<?php echo $edituser; ?>" placeholder="Enter Username"/></td>
         </tr>
         <tr>
           <td>First Name: </td>
